...
# Use ulab for SAMD51
if (sys.platform == 'MicroChip SAMD51'):                # Identify board
    K =  np.array([18.046, -3972.5, -93.018, 33.947])   # LQ gain with integrator, see MATLAB example
    X =  np.zeros((4, 1))                               # Initial state vector
    Xr = np.zeros((4, 1))                               # Initial state reference
else:                                                   # else just use lists
    K =  [18.046, -3972.5, -93.018, 33.947]             # LQ gain with integrator, see MATLAB example
...
def fallbackSettings():                                 # Used later, comment out the function call if not needed.
    ...
    if (microcontroller.cpu.frequency/1000000 == 48):   # For the Adafruit Metro M0 @48 MHz override defaults
        ...
        K = [7.0461, -3565.1, -82.216, 29.996]          # Less aggressive LQ going to make it work (at least on some level)
...
# LQ control algorithm
    if (sys.platform == 'MicroChip SAMD51'):                # Identify board
        u = -(linalg.dot(K, X) + u0)[0]                     # LQ control algorithm using ulab
    else:
        u = 0.0                                             # Initialize u
        for iVec in range(0, 4):                            # For each element in K
            u = u - K[iVec]*X[iVec]                         # Nominal LQ part
