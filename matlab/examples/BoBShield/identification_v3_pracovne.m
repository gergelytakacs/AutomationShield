...
%validation
%%
Hh=model.Parameters(1).Value;
ydata=dataAll(5800:6200,2)/1000;
ydata=detrend(ydata);
Ts=1;
A = [0 1 ; 0 0];
Bmodel = [0 Hh]';
C=[1 0];
D=0;
mymodel=ss(A,Bmodel,C,D);
dmodel=c2d(mymodel,0.01,'toc')
A=dmodel.A;
B=dmodel.B
u=dataAll(5800:6200,1)*pi/180;
n=length(u);
x0data=[ydata(1);(ydata(2)-ydata(1))/Ts];
Hh=model.Parameters(1).Value;
x=x0data;
