...
%data = data(240:550)                             % Discard the time when magnet is on ground, pick close to linearization point
%data = data(1:800)                             % Discard the time when magnet is on ground, pick close to linearization point

%data = data(1:1000)                             % Discard the time when magnet is on ground, pick close to linearization point
data=detrend(data);
...
%https://www.tandfonline.com/doi/abs/10.1080/00207729608929322
...
Options.WeightingFilter = [0 700];   
Options.EnforceStability = 0;  % Unstable is okay, that's what we have                      
Options.InitialState = 'estimate';  
Options.SearchMethod='lsqnonlin' %gn,gna,lm,grad,lsqnonlin,fmincon
%Options.Regularization.Lambda=1E-10;
Options.SearchOptions.MaxIterations=200;
Options.SearchOptions.StepTolerance=1E-12;
Options.SearchOptions.FunctionTolerance=1E-12;
