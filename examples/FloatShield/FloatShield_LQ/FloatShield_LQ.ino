/*
  ...
  the AutomationShield library, calculates LQ gain through
  provided state-space matrices and then with the help of
...
BLA::Matrix<3, 3> A = {1.0, 0.047728537419688, 0, 0, 0.958418172068238, 0.890399017362750, 0, 0, 0.981342498795679};
BLA::Matrix<3, 1> B = {0, 0.156853676350520, 0};
BLA::Matrix<1, 3> C = {1, 0, 0};

// Kalman process and measurement error covariances (utilizing matrices Q and R for two different purposes)
BLA::Matrix<3, 3> Q_Kalman = {1, 0, 0, 0, 1, 0, 0, 0, 1};     // Process error covariance matrix
BLA::Matrix<1, 1> R_Kalman = {10};                            // Measurement error covariance matrix

// Calculating LQ gain
BLA::Matrix<3, 3> Q_LQ = ~C * C;                                // State penalisation matrix
BLA::Matrix<1, 1> R_LQ = {1};                                   // Input penalisation matrix
BLA::Matrix<1, 3> K = FloatShield.getGainLQ(A, B, Q_LQ, R_LQ);  // Calculate gain K
...
void setup() {                         // Setup - runs only once
    ...
    while(1) {                                  // Wait for ball to lift off from ground
        ...
        X = FloatShield.getKalmanEstimate(u, y, A, B, C, Q_Kalman, R_Kalman);     // Estimate internal states X        
